...
describeWithShallowAndMount('contains', mountingMethod => {
  ...
  it('returns true if wrapper contains Kdu component', () => {
    const wrapper = mountingMethod(ComponentWithChild)
    expect(wrapper.contains(Component)).to.equal(true)
  })

  it('returns true if wrapper contains functional Kdu component', () => {
  ...
  itSkipIf(
    isRunningPhantomJS,
    'returns true if wrapper contains Kdu class component',
    () => {
      const TestComponent = {
        template: `
        <div>
  ...
  it('throws an error if selector is not a valid selector', () => {
    ...
    invalidSelectors.forEach(invalidSelector => {
      const message =
        '[kdu-test-utils]: wrapper.contains() must be passed a valid CSS selector, Kdu constructor, or valid find option object'
