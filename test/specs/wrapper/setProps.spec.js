...
describeWithShallowAndMount('setProps', mountingMethod => {
  ...
  it('sets props and updates when called with same object', async () => {
    const TestComponent = {
      template: '<div k-if="obj && obj.shouldRender()" />',
      props: ['obj']
    }
    const obj = {
      shouldRender: () => false
  ...
  describe('invalid arguments', () => {
    const errors = {
      FUNCTIONAL_COMPONENT_ERROR: `[kdu-test-utils]: wrapper.setProps() cannot be called on a functional component`,
      SAME_REFERENCE_ERROR: `[kdu-test-utils]: wrapper.setProps() called with the same object of the existing obj property. You must call wrapper.setProps() with a new object to trigger reactivity`,
      INVALID_NODE_ERROR: `wrapper.setProps() can only be called on a Kdu instance`,
      WRONG_PROP_ERROR: `[kdu-test-utils]: wrapper.setProps() called with prop1 property which is not defined on the component`
    }
