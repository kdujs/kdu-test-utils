import ComponentWithInput from '~resources/components/component-with-input.kdu'
...
describeWithShallowAndMount('setChecked', mountingMethod => {
  ...
  it('updates dom with checkbox k-model', async () => {
    const wrapper = mountingMethod(ComponentWithInput)
    const input = wrapper.find('input[type="checkbox"]')
  ...
  it('changes state the right amount of times with checkbox k-model', async () => {
    const wrapper = mountingMethod(ComponentWithInput)
    const input = wrapper.find('input[type="checkbox"]')
  ...
  it('triggers a change event when called on a checkbox', () => {
    ...
    mountingMethod({
      // For compatibility with earlier versions of Kdu that use the `click`
      // event for updating `k-model`.
      template: `
        <input
          type="checkbox"
          @change="listener"
  ...
  it('updates dom with radio k-model', async () => {
    const wrapper = mountingMethod(ComponentWithInput)
  ...
  it('changes state the right amount of times with radio k-model', async () => {
    const wrapper = mountingMethod(ComponentWithInput)
  ...
  it('throws error if checked param is not boolean', () => {
    ...
    expect(fn)
      .to.throw()
      .with.property('message', '[kdu-test-utils]: ' + message)
  })
