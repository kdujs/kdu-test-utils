import { Wrapper, WrapperArray } from '@kdujs/test-utils'
...
describeWithShallowAndMount('WrapperArray', mountingMethod => {
  ...
  ;['wrappers', 'length'].forEach(property => {
    it(`has the ${property} property which is read-only`, () => {
      ...
      const message = `[kdu-test-utils]: wrapperArray.${property} is read-only`
      expect(() => {
  ...
  methods.forEach(method => {
    it(`throws error if ${method} is called when there are no items in wrapper array`, () => {
      ...
      const wrapperArray = getWrapperArray([])
      const message = `[kdu-test-utils]: ${method} cannot be called on 0 items`
    ...
    it(`${method} throws error if called when there are items in wrapper array`, () => {
      ...
      const message = `[kdu-test-utils]: ${method} must be called on a single wrapper, use at(i) to access a wrapper`
      expect(() => wrapperArray[method]())
        .to.throw()
        .with.property('message', message)
    })
  ...
  it('isKduInstance returns true if every wrapper.isKduInstance() returns true', () => {
    const wrapperArray = getWrapperArray([
      { isKduInstance: () => true },
      { isKduInstance: () => true }
    ])
    expect(wrapperArray.isKduInstance()).to.equal(true)
  })

  it('isKduInstance returns false if not every wrapper.isKduInstance() returns true', () => {
    const wrapperArray = getWrapperArray([
      { isKduInstance: () => true },
      { isKduInstance: () => false }
    ])
    expect(wrapperArray.isKduInstance()).to.equal(false)
  })
