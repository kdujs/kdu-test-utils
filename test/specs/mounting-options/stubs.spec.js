...
import ComponentWithNestedChildrenAndAttributes from '~resources/components/component-with-nested-childern-and-attributes.kdu'
import { createLocalKdu, config } from '@kdujs/test-utils'
import { config as serverConfig } from '@kdujs/server-test-utils'
import Kdu from 'kdu'
...
describeWithShallowAndMount('options.stub', mountingMethod => {
  ...
  it('accepts valid component stubs', () => {
    ...
    const SubclassedComponent = Kdu.extend({ template: '<div></div>' })
  ...
  itSkipIf(
    ...
    () => {
      const extendedComponent = Kdu.extend({
  ...
  itDoNotRunIf(
    mountingMethod.name === 'shallowMount' || kduVersion < 2.3,
    ...
    () => {
      ...
      Kdu.component('grand-child-component', GrandChildComponent)
      ...
      delete Kdu.options.components['grand-child-component']
    }
  )
  ...
  itDoNotRunIf(
    ...
    () => {
      ...
      const wrapper = mountingMethod(Kdu.extend(TestComponent), {
        stubs: ['grand-child-component']
      })
      expect(wrapper.html()).not.to.contain('<span>')
    }
  ...
  it('stubs components with dummy when passed as an array', () => {
    ...
    const error =
      '[kdu-test-utils]: each item in an options.stubs array must be a string'
  ...
  it('throws error if passed string in object when kdu-template-compiler is undefined', () => {
    const compilerSave =
      require.cache[require.resolve('kdu-template-compiler')].exports
        .compileToFunctions
    require.cache[
      require.resolve('kdu-template-compiler')
    ].exports.compileToFunctions = undefined
    delete require.cache[require.resolve('@kdujs/test-utils')]
    delete require.cache[require.resolve('@kdujs/server-test-utils')]
    const mountingMethodFresh = require('@kdujs/test-utils')[
      mountingMethod.name
    ]
    const message =
      '[kdu-test-utils]: kduTemplateCompiler is undefined, you must pass precompiled components if kdu-template-compiler is undefined'
    const fn = () =>
  ...
  it('combines with stubs from config', () => {
    ...
    localKdu.component('span-component', SpanComponent)
    localKdu.component('time-component', TimeComponent)
    const TestComponent = {
  ...
  it('throws an error when passed a circular reference for string stubs', () => {
    ...
    const error =
      '[kdu-test-utils]: options.stub cannot contain a circular reference'
    names.forEach(name => {
      invalidValues.forEach(invalidValue => {
        const fn = () =>
          mountingMethod(ComponentWithChild, {
  ...
  it('throws an error when passed an invalid value as stub', () => {
    const error =
      '[kdu-test-utils]: options.stub values must be passed a string or component'
  ...
  itRunIf(
    ...
    () => {
      ...
      const message =
        `[kdu-test-utils]: options.stubs is not supported for components ` +
        `created with Kdu.extend in Kdu < 2.3. You can set stubs to false ` +
  ...
  it('works with async components', () => {
    ...
    const TestComponent = {
      ...
      components: {
        DynamicHello: () => import('~resources/components/component.kdu'),
        DynamicHello2: () => import('~resources/components/component.kdu'),
        DynamicHello3: () => import('~resources/components/component.kdu')
      }
    }
    const wrapper = mountingMethod(TestComponent, {
      stubs: {
  ...
  it('warns when passing a string', () => {
    ...
    expect(console.error).calledWith(
      sandbox.match('[kdu-test-utils]: Using a string for stubs is deprecated')
    )
