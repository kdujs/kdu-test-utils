...
describeWithShallowAndMount('options.mocks', mountingMethod => {
  ...
  itSkipIf(kduVersion < 2.3, 'adds variables to extended components', () => {
  ...
  itDoNotRunIf(
    ...
    () => {
      ...
      const wrapper = mountingMethod(
        {
          template: '<div><component-with-kdux /></div>',
          components: {
            ComponentWithKdux
          }
        },
  ...
  itDoNotRunIf(
    mountingMethod.name === 'shallowMount',
    'adds variables available to nested vms using localKdu',
    () => {
  ...
  it('does not affect global kdu class when passed as mocks object', () => {
    const $store = { store: true }
  ...
  it('logs that a property cannot be overwritten if there are problems writing', () => {
    const localKdu = createLocalKdu()
    Object.defineProperty(localKdu.prototype, '$store', {
      value: 42
    })
    const $store = 64
    ...
    const msg =
      `[kdu-test-utils]: could not overwrite property $store, this ` +
  ...
  itRunIf(
    ...
    () => {
      ...
      const message =
        `[kdu-test-utils]: options.mocks is not supported for components ` +
        `created with Kdu.extend in Kdu < 2.3. You can set mocks to false ` +
