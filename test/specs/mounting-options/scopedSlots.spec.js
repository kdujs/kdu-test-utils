...
import ComponentWithScopedSlots from '~resources/components/component-with-scoped-slots.kdu'
...
describeWithShallowAndMount('scopedSlots', mountingMethod => {
  ...
  itDoNotRunIf(kduVersion < 2.1, 'handles templates as the root node', () => {
  ...
  itDoNotRunIf(kduVersion < 2.1, 'handles render functions', () => {
    const wrapper = mountingMethod(
      {
  ...
  itDoNotRunIf(kduVersion < 2.5, 'mounts component scoped slots', async () => {
  ...
  itDoNotRunIf(kduVersion < 2.5, 'handles JSX', () => {
    const wrapper = mountingMethod(
      {
  ...
  itDoNotRunIf(kduVersion < 2.5, 'handles no slot-scope', () => {
    const wrapper = mountingMethod(
      {
        template: '<div><slot name="single" :text="foo" :i="123"></slot></div>',
        data: () => ({
  ...
  itDoNotRunIf(
    kduVersion > 2.0,
    'throws exception when kdu version < 2.1',
    () => {
      const fn = () => {
      ...
      const message =
        '[kdu-test-utils]: the scopedSlots option is only supported in kdu@2.1+.'
      expect(fn)
        .to.throw()
  ...
  itDoNotRunIf(
    kduVersion < 2.6,
    'renders scoped slot with k-slot syntax',
    () => {
      const TestComponent = {
        data() {
          return {
      ...
      const wrapper = mountingMethod(TestComponent, {
        scopedSlots: {
          default:
            '<template k-slot:default="{ val }"><p>{{ val }}</p></template>',
          named:
            '<template k-slot:named="prop"><p>{{ prop.val }}</p></template>'
        }
      })
      expect(wrapper.html()).to.equal('<div>\n  <p>25</p>\n  <p>50</p>\n</div>')
    }
  ...
  itDoNotRunIf(
    kduVersion < 2.6,
    'renders scoped slot with shorthand k-slot syntax',
    () => {
      const TestComponent = {
        data() {
          return {
  ...
  itDoNotRunIf(
    kduVersion < 2.5 || mountingMethod.name !== 'mount',
    'renders using localKdu constructor',
    () => {
      ...
      localKdu.component('registered-component', RegisteredComponent)
  ...
  itDoNotRunIf(
    ...
    () => {
      const wrapper = mountingMethod(
        ...
        {
          scopedSlots: {
            single: '<input k-model="props.text" type="text" />'
          }
        }
      )
