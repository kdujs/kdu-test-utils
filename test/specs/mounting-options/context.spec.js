...
describeWithShallowAndMount('options.context', mountingMethod => {
  ...
  it('throws error if non functional component is passed with context option', () => {
    ...
    const context = {}
    const message =
      '[kdu-test-utils]: mount.context can only be used when mounting a functional component'
  ...
  it('does not throw error if functional component with Kdu.extend', () => {
  ...
  it('throws error if context option is not an object', () => {
    ...
    const message = '[kdu-test-utils]: mount.context must be an object'
    const fn = () => mountingMethod(Component, { context })
    expect(fn)
      .to.throw()
      .with.property('message', message)
  ...
  it('mounts functional component with a defined context when no context object passed in options', () => {
    const defaultValue = '[kdu-test-utils]: testProp default value'
    const Component = {
      functional: true,
      props: {
        testProp: {
