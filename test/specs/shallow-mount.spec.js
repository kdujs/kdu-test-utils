...
import Kdu from 'kdu'
import { mount, shallowMount, createLocalKdu } from '@kdujs/test-utils'
...
import ComponentWithFunctionalChild from '~resources/components/component-with-functional-child.kdu'
import ComponentWithNestedChildren from '~resources/components/component-with-nested-children.kdu'
import ComponentWithLifecycleHooks from '~resources/components/component-with-lifecycle-hooks.kdu'
import ComponentWithoutName from '~resources/components/component-without-name.kdu'
import ComponentAsAClassWithChild from '~resources/components/component-as-a-class-with-child.kdu'
import ComponentWithKSlotSyntax from '~resources/components/component-with-k-slot-syntax.kdu'
import ComponentWithKSlot from '~resources/components/component-with-k-slot.kdu'
import RecursiveComponent from '~resources/components/recursive-component.kdu'
import { kduVersion } from '~resources/utils'
...
describeRunIf(process.env.TEST_ENV !== 'node', 'shallowMount', () => {
  ...
  it('returns new KduWrapper of Kdu localKdu if no options are passed', () => {
  ...
  it('returns new KduWrapper with all children stubbed', () => {
  ...
  it('stubs globally registered components when options.localKdu is provided', () => {
    const localKdu = Kdu.extend()
    localKdu.component('registered-component', ComponentWithLifecycleHooks)
    const TestComponent = {
      render: h => h('registered-component')
    }
    shallowMount(TestComponent, { localKdu })
    localKdu.component('registered-component', ComponentWithLifecycleHooks)
    mount(TestComponent, { localKdu })
  ...
  it('renders SFC with named slots with k-slot syntax', () => {
    const wrapper = shallowMount(ComponentWithKSlotSyntax)

    expect(wrapper.find(ComponentWithKSlot).exists()).to.equal(true)
    expect(wrapper.find('.new-example').exists()).to.equal(true)
    expect(wrapper.html()).to.equal(
      '<componentwithkslot-stub>\n' +
        '  <p class="new-example">new slot syntax</p>\n' +
        '</componentwithkslot-stub>'
    )
  })

  it('renders named slots with k-slot syntax', () => {
    const localKdu = createLocalKdu()
    localKdu.component('Foo', {
      template: '<div><slot name="newSyntax" /></div>'
    })
    const TestComponent = {
      template: `
        <Foo>
          <template k-slot:newSyntax>
  ...
  it('stubs globally registered components', () => {
    Kdu.component('registered-component', ComponentWithLifecycleHooks)
    const Component = {
      render: h => h('registered-component')
    }
  ...
  itDoNotRunIf(
    kduVersion < 2.2, // $props does not exist in Kdu < 2.2
    'renders stubs props',
    () => {
      const TestComponent = {
  ...
  itDoNotRunIf(kduVersion < 2.1, 'handles recursive components', () => {
    const TestComponent = {
      template: `
          <div>
            <test-component />
    ...
    expect(console.error).not.calledWith('[Kdu warn]')
  })
  ...
  it('stubs components that receive props through mixin', () => {
    ...
    const ChildComponentExtended = Kdu.extend({
      template: '<div />',
      mixins: [addProps]
    })
  ...
  itDoNotRunIf(kduVersion < 2.3, 'stubs Kdu class component children', () => {
  ...
  it('stubs components registered on localKdu after multiple installs', () => {
    const myPlugin = function(_Kdu, opts) {
      _Kdu.mixin({})
    }
    const localKdu = createLocalKdu()
    localKdu.component('registered-component', {
      render: h => h('time')
    })
    const TestComponent = {
      render: h => h('registered-component')
    ...
    localKdu.use(myPlugin)
    const wrapper = shallowMount(TestComponent, { localKdu })
  ...
  itDoNotRunIf(
    ...
    () => {
      ...
      `,
        components: {
          ExtendedComponent: Kdu.extend({ render: h => h('div') }),
          ChildComponent
        }
      }
      shallowMount(TestComponent)
  ...
  itDoNotRunIf(kduVersion < 2.2, 'stubs model', () => {
    const ChildComponent = {
      template: '<div />',
      model: {
        prop: 'a',
    ...
    const TestComponent = {
      template: '<child-component k-model="val" />',
      data() {
        return {
          val: '123'
        }
