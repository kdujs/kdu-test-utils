...
export default class Wrapper implements BaseWrapper {
  ...
  isKduComponent: boolean;
  element: Element;
  update: Function;
  options: WrapperOptions;
  version: number

  constructor (node: KNode | Element, update: Function, options: WrapperOptions) {
    ...
    this.version = Number(`${Kdu.version.split('.')[0]}.${Kdu.version.split('.')[1]}`)
  }
  ...
  contains (selector: Selector) {
    ...
    const nodes = findAll(this.vm, this.knode, this.element, selector)
  ...
  emittedByOrder () {
    if (!this._emittedByOrder && !this.vm) {
      throwError('wrapper.emittedByOrder() can only be called on a Kdu instance')
    }
  ...
  hasAttribute (attribute: string, value: string) {
    warn('hasAttribute() has been deprecated and will be removed in version 1.0.0. Use attributes() instead—https://kdujs-test-utils.web.app/en/api/wrapper/attributes')
  ...
  hasClass (className: string) {
    warn('hasClass() has been deprecated and will be removed in version 1.0.0. Use classes() instead—https://kdujs-test-utils.web.app/en/api/wrapper/classes')
  ...
  hasProp (prop: string, value: string) {
    warn('hasProp() has been deprecated and will be removed in version 1.0.0. Use props() instead—https://kdujs-test-utils.web.app/en/api/wrapper/props')

    if (!this.isKduComponent) {
      throwError('wrapper.hasProp() must be called on a Kdu instance')
    }
    if (typeof prop !== 'string') {
      throwError('wrapper.hasProp() must be passed prop as a string')
    }

    // $props object does not exist in Kdu 2.1.x, so use $options.propsData instead
  ...
  hasStyle (style: string, value: string) {
    ...
    if (!this.options.attachedToDocument && (this.vm || this.knode)) {
      // $FlowIgnore : Possible null value, will be removed in 1.0.0
      const vm = this.vm || this.knode.context.$root
      body.insertBefore(vm.$root._knode.elm, null)
    }
   ...
   */
  find (selector: Selector): Wrapper | ErrorWrapper | KduWrapper {
    const nodes = findAll(this.vm, this.knode, this.element, selector)
    if (nodes.length === 0) {
      if (selector.ref) {
  ...
  is (selector: Selector): boolean {
    ...
    if (selectorType === COMPONENT_SELECTOR) {
      ...
      if (selector.functional) {
        return vmFunctionalCtorMatchesSelector(this.vm._knode, selector._Ctor)
      }
  ...
  isEmpty (): boolean {
    ...
    return this.knode.children === undefined || this.knode.children.length === 0
  }
  ...
  setComputed (computed: Object) {
    if (!this.isKduComponent) {
      throwError('wrapper.setComputed() can only be called on a Kdu instance')
    }
    ...
    Object.keys(computed).forEach((key) => {
      if (this.version > 2.1) {
        ...
        if (!this.vm._computedWatchers[key]) {
          throwError(`wrapper.setComputed() was passed a value that does not exist as a computed property on the Kdu instance. Property ${key} does not exist on the Kdu instance`)
        }
        // $FlowIgnore : Problem with possibly null this.vm
        ...
        this.vm._watchers.forEach(watcher => {
          if (watcher.getter.kdux && key in watcher.vm.$options.store.getters) {
  ...
  setProps (data: Object) {
    if (!this.isKduComponent || !this.vm) {
  ...
  destroy () {
    if (!this.isKduComponent) {
      throwError('wrapper.destroy() can only be called on a Kdu instance')
    }
  ...
  trigger (type: string, options: Object = {}) {
    ...
    if (options.target) {
      throwError('you cannot set the target value of an event. See the notes section of the docs for more details—https://kdujs-test-utils.web.app/en/api/wrapper/trigger.html')
    }
