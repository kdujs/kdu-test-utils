(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('kdu'), require('kdu-template-compiler'), require('@kdujs/test-utils')) :
  typeof define === 'function' && define.amd ? define(['exports', 'kdu', 'kdu-template-compiler', '@kdujs/test-utils'], factory) :
  (global = global || self, factory(global.KduTestUtils = {}, global.Kdu, global.KduTemplateCompiler, global.testUtils));
}(this, (function (exports, Kdu, kduTemplateCompiler, testUtils) { 'use strict';

  Kdu = Kdu && Kdu.hasOwnProperty('default') ? Kdu['default'] : Kdu;

  // 

  function createKNodes(vm, slotValue, name) {
    var el = kduTemplateCompiler.compileToFunctions(
    ...
    var knode = el.render.call(vm._renderProxy, vm.$createElement);
  ...
  function createKNodesForSlot(
    ...
    var knode = vm.$createElement(slotValue)
    ;(knode.data || (knode.data = {})).slot = name;
    return knode
  }

  function createSlotKNodes(
    vm,
    slots
  ) {
    return Object.keys(slots).reduce(function (acc, key) {
      ...
      if (Array.isArray(content)) {
        var nodes = content.map(function (slotDef) { return createKNodesForSlot(vm, slotDef, key); }
        );
  ...
  var BEFORE_RENDER_LIFECYCLE_HOOK = semver.gt(Kdu.version, '2.1.8')
    ? 'beforeCreate'
    : 'beforeMount';

  var CREATE_ELEMENT_ALIAS = semver.gt(Kdu.version, '2.1.5')
    ? '_c'
    : '_h';
  ...
  function addStubs(_Kdu, stubComponents) {
    var obj;
    ...
    _Kdu.mixin(( obj = {}, obj[BEFORE_RENDER_LIFECYCLE_HOOK] = addStubComponentsMixin, obj ));
  }
  ...
  function compileTemplate(component) {
    if (component.template) {
      if (!kduTemplateCompiler.compileToFunctions) {
        throwError(
      ...
      Object.assign(component, Object.assign({}, kduTemplateCompiler.compileToFunctions(component.template),
        {name: component.name}));
    }
  ...
  function isScopedSlot(slot) {
    ...
    var kSlotRe = /<template k-slot(?::.+)?="(.+)"/;
    var shortKSlotRe = /<template #.*="(.+)"/;

    var hasOldSlotScope = slot.match(slotScopeRe);
    var hasKSlotScopeAttr = slot.match(kSlotRe);
    var hasShortKSlotScopeAttr = slot.match(shortKSlotRe);
  ...
  function createScopedSlots(
    ...
    validateEnvironment();
    var helpers = getKduTemplateCompilerHelpers(_Kdu);
    var loop = function ( scopedSlotName ) {
      ...
      var renderFn =
        typeof slot === 'function'
          ? slot
          : kduTemplateCompiler.compileToFunctions(scopedSlotMatches.slot, { warn: customWarn })
              .render;
  ...
  function isKduComponentStub(comp) {
  ...
  function getScopedSlotRenderFunctions(ctx) {
    ...
    if (
      ...
      var slotKeys = ctx.$options.parent._knode.data.scopedSlots;
  ...
  function createStubFromComponent(
    ...
    var componentOptions = resolveOptions(originalComponent, _Kdu);
        ...
        return h(
          ...
          context
            ...
            : this.$options._renderChildren ||
                getScopedSlotRenderFunctions(this).map(function (x) { return this$1.$options.parent._knode.data.scopedSlots[x](); }
                )
        )
      }})
  }
  ...
  function extend(component, _Kdu) {
    ...
    var stub = _Kdu.extend(componentOptions);
  ...
  function patchCreateElement(_Kdu, stubs, stubAllComponents) {
    var obj;
      ...
      var createElement = function (el) {
        ...
        if (isConstructor(el) || isComponentOptions(el)) {
          if (stubAllComponents) {
            var stub = createStubFromComponent(el, el.name || 'anonymous', _Kdu);
            return originalCreateElement.apply(void 0, [ stub ].concat( args ))
          }
          var Constructor = shouldExtend(el) ? extend(el, _Kdu) : el;
        ...
        if (typeof el === 'string') {
          ...
          var stub$1 = createStubIfNeeded(stubAllComponents, original, _Kdu, el);
    ...
    _Kdu.mixin(( obj = {}, obj[BEFORE_RENDER_LIFECYCLE_HOOK] = patchCreateElementMixin, obj ));
  }
  ...
  function createChildren(vm, h, ref) {
    ...
    var slotKNodes = slots ? createSlotKNodes(vm, slots) : undefined;
    return (
      (context &&
        context.children &&
  ...
  function createInstance(
    ...
    var globalComponents = _Kdu.options.components || {};
    ...
    var Constructor = _Kdu.extend(componentOptions).extend(instanceOptions);
    Constructor.options._base = _Kdu;

    var scopedSlots = createScopedSlots(options.scopedSlots, _Kdu);
    ...
    var Parent = _Kdu.extend(parentComponentOptions);
  ...
  function vmMatchesName(vm, name) {
    // We want to mirror how Kdu resolves component names in SFCs:
  ...
  function vmCtorMatches(vm, component) {
    if (
      (vm.$options && vm.$options.$_kduTestUtils_original === component) ||
      vm.$_kduTestUtils_original === component
    ) {
      return true
    }
  ...
  function matches(node, selector) {
    ...
    // Fallback to name selector for COMPONENT_SELECTOR for Kdu < 2.1
  ...
  function findAllKNodes(knode, selector) {
    var matchingNodes = [];
    var nodes = [knode];
    while (nodes.length) {
  ...
  function removeDuplicateNodes(kNodes) {
    var kNodeElms = kNodes.map(function (kNode) { return kNode.elm; });
    return kNodes.filter(function (kNode, index) { return index === kNodeElms.indexOf(kNode.elm); })
  }
  ...
  function find(
    ...
    var nodes = findAllKNodes(root, selector);
  ...
  // instances threw.
  function addGlobalErrorHandler(_Kdu) {
    var existingErrorHandler = _Kdu.config.errorHandler;
  ...
  function normalizeProvide(provide) {
    // Objects are not resolved in extended components in Kdu < 2.5
    if (typeof provide === 'object' && KDU_VERSION < 2.5) {
  ...
  **/
  ...
  WrapperArray.prototype.isKduInstance = function isKduInstance () {
    this.throwErrorIfWrappersIsEmpty('isKduInstance');

    return this.wrappers.every(function (wrapper) { return wrapper.isKduInstance(); })
  };
  ...
  ErrorWrapper.prototype.isKduInstance = function isKduInstance () {
    throwError(
      ("find did not return " + (buildSelectorString(
        this.selector
      )) + ", cannot call isKduInstance() on empty Wrapper")
    );
  };
  ...
  var Wrapper = function Wrapper(
    ...
    var knode = node instanceof Element ? null : node;
    var element = node instanceof Element ? node : node.elm;
    // Prevent redefine by KduWrapper
    if (!isKduWrapper) {
      // $FlowIgnore : issue with defineProperty
      Object.defineProperty(this, 'rootNode', {
        get: function () { return knode || element; },
        set: function () { return throwError('wrapper.rootNode is read-only'); }
      });
      // $FlowIgnore
      Object.defineProperty(this, 'knode', {
        get: function () { return knode; },
        set: function () { return throwError('wrapper.knode is read-only'); }
      });
      // $FlowIgnore
    ...
    if (
      this.knode &&
      (this.knode[FUNCTIONAL_OPTIONS] || this.knode.functionalContext)
    ) {
  ...
  Wrapper.prototype.destroy = function destroy () {
    if (!this.isKduInstance() && !this.isFunctionalComponent) {
      throwError(
        "wrapper.destroy() can only be called on a Kdu instance or " +
  ...
  Wrapper.prototype.emitted = function emitted (
    ...
    if (!this._emitted && !this.vm) {
      throwError("wrapper.emitted() can only be called on a Kdu instance");
    }
    if (event) {
  ...
  Wrapper.prototype.emittedByOrder = function emittedByOrder () {
    ...
    if (!this._emittedByOrder && !this.vm) {
      throwError(
        "wrapper.emittedByOrder() can only be called on a Kdu instance"
      );
    }
  ...
  Wrapper.prototype.findComponent = function findComponent (rawSelector) {
    ...
    if (!this.vm) {
      throwError(
        'You cannot chain findComponent off a DOM element. It can only be used on Kdu Components.'
      );
    }

    if (selector.type === DOM_SELECTOR) {
      throwError(
        'findComponent requires a Kdu constructor or valid find object. If you are searching for DOM nodes, use `find` instead'
      );
    }
  ...
  Wrapper.prototype.findAllComponents = function findAllComponents (rawSelector) {
    ...
    if (!this.vm) {
      throwError(
        'You cannot chain findAllComponents off a DOM element. It can only be used on Kdu Components.'
      );
    }
    if (selector.type === DOM_SELECTOR) {
      throwError(
        'findAllComponents requires a Kdu constructor or valid find object. If you are searching for DOM nodes, use `find` instead'
      );
    }
    return this.__findAll(rawSelector, selector)
  };
  ...
  Wrapper.prototype.__findAll = function __findAll (rawSelector, selector) {
    ...
    var wrappers = nodes.map(function (node) {
      // Using CSS Selector, returns a KduWrapper instance if the root element
      // binds a Kdu instance.
  ...
  /**
   * Checks if wrapper is a kdu instance
   * @deprecated
   */
  Wrapper.prototype.isKduInstance = function isKduInstance () {
    warnDeprecated("isKduInstance");
    return !!this.vm
  };

  /**
   * Returns name of component, or tag name if node is not a Kdu component
   * @deprecated
   */
  ...
  Wrapper.prototype.overview = function overview () {
    ...
    if (!this.isKduInstance()) {
      throwError("wrapper.overview() can only be called on a Kdu instance");
    }
  ...
  Wrapper.prototype.props = function props (key) {
    ...
    if (!this.vm) {
      throwError('wrapper.props() must be called on a Kdu instance');
    }
  ...
  Wrapper.prototype.setChecked = function setChecked (checked) {
    ...
    warnDeprecated(
      "setChecked",
      'When you migrate to KTU 2, use setValue instead.'
    );
  ...
  Wrapper.prototype.setData = function setData (data) {
    ...
    if (!this.vm) {
      throwError("wrapper.setData() can only be called on a Kdu instance");
    }
  ...
  Wrapper.prototype.setMethods = function setMethods (methods) {
    ...
    if (!this.isKduInstance()) {
      throwError("wrapper.setMethods() can only be called on a Kdu instance");
    }
    ...
    if (this.knode) {
      var context = this.knode.context;
  ...
  Wrapper.prototype.setProps = function setProps (data) {
    ...
    if (!this.vm) {
      throwError("wrapper.setProps() can only be called on a Kdu instance");
    }

    // Save the original "silent" config so that we can directly mutate props
    var originalConfig = Kdu.config.silent;
    Kdu.config.silent = config.silent;
    ...
    try {
      ...
      Kdu.config.silent = originalConfig;
    }
  };
  ...
  Wrapper.prototype.setValue = function setValue (value) {
    ...
    if (tagName === 'OPTION') {
      ...
      // for k-model.lazy, we need to trigger a change event, too.
      // $FlowIgnore
      if (
        (tagName === 'INPUT' || tagName === 'TEXTAREA') &&
        this.element._kModifiers &&
        this.element._kModifiers.lazy
      ) {
        this.trigger('change');
      }
      return nextTick()
  ...
  var KduWrapper = /*@__PURE__*/(function (Wrapper) {
    function KduWrapper(vm, options) {
      var this$1 = this;

      Wrapper.call(this, vm._knode, options, true);
      // $FlowIgnore : issue with defineProperty
      Object.defineProperty(this, 'rootNode', {
        get: function () { return vm.$knode || { child: this$1.vm }; },
        set: function () { return throwError('wrapper.knode is read-only'); }
      });
      // $FlowIgnore : issue with defineProperty
      Object.defineProperty(this, 'knode', {
        get: function () { return vm._knode; },
        set: function () { return throwError('wrapper.knode is read-only'); }
      });
      // $FlowIgnore
      Object.defineProperty(this, 'element', {
    ...
    if ( Wrapper ) KduWrapper.__proto__ = Wrapper;
    KduWrapper.prototype = Object.create( Wrapper && Wrapper.prototype );
    KduWrapper.prototype.constructor = KduWrapper;
  ...
  function createWrapper(
    ...
    if (componentInstance) {
      var wrapper$1 = new KduWrapper(componentInstance, options);
  ...
  function createLocalKdu(_Kdu) {
    if ( _Kdu === void 0 ) _Kdu = Kdu;

    var instance = _Kdu.extend();

    // clone global APIs
    Object.keys(_Kdu).forEach(function (key) {
      if (!instance.hasOwnProperty(key)) {
        var original = _Kdu[key];
    ...
    instance.config = cloneDeep_1(Kdu.config);
  ...
  function requiresTemplateCompiler(slot) {
    if (typeof slot === 'string' && !kduTemplateCompiler.compileToFunctions) {
      throwError(
        "kduTemplateCompiler is undefined, you must pass " +
          "precompiled components if kdu-template-compiler is " +
          "undefined"
  ...
  // correctly
  var UNSUPPORTED_VERSION_OPTIONS = ['mocks', 'stubs', 'localKdu'];
  ...
  function mount(component, options) {
    ...
    var _Kdu = createLocalKdu(options.localKdu);

    var mergedOptions = mergeOptions(options, config);

    validateOptions(mergedOptions, component);

    var parentVm = createInstance(component, mergedOptions, _Kdu);
  ...
  exports.createLocalKdu = createLocalKdu;
